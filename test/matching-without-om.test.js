...
class BasicExpr {
    constructor (arg) {
        this.children = arg instanceof Array ? arg : [ ]
        this.text = typeof( arg ) === 'string' ? arg : ''
        this.parent = null
        this.children.map( child => {
            if (child instanceof BasicExpr) child.parent = this 
        } )
    }
    type () {
        return this.text.length > 0 ? 'v' : this.children[0]
    }
    copy () {
        const result = this.children.length > 0 ?
            BE( [ this.children[0], ...this.children.slice( 1 ).map( x => x.copy() ) ] ) :
            BE( this.text )
        result.children.slice( 1 ).map( x => x.parent = result )
        return result
    }
    toJSON () {
        if ( this.text.length > 0 ) {
            if ( this.children.length > 0 ) {
                return `ERROR-${this.text}-${this.children[0]}-`
                     + `${this.children.slice(1).map(x=>x.toJSON()).join(',')}`
            }
            return this.text
        }
        let result = ''
        result += this.children[0] === 'a' ? '(' :
                  this.children[0] === 'b' ? '[' : '?'
        result += this.children.slice( 1 ).map(x=>x.toJSON()).join(',')
        result += this.children[0] === 'a' ? ')' :
                  this.children[0] === 'b' ? ']' : '?'
        return result
    }
    simpleEncode () { return this.toJSON() }
}
const BE = arg => new BasicExpr( arg )
const EFA = (...args) => M.makeExpressionFunctionApplication( ...args )
const EF = (...args) => M.makeExpressionFunction(args.slice(0,args.length-1),args[args.length-1])
const API = {
    isExpression : (e) => e instanceof BasicExpr,
    filterSubexpressions : (e,f) => ( f(e) ? [ e ] : [ ] ).concat(
        API.getChildren(e).map( c => API.filterSubexpressions(c,f) ).flat() ),
    sameType : (e1,e2) => e1.type() === e2.type(),
    copy : (e) => e.copy(),
    equal : (e1,e2) =>
        API.isExpression(e1) && API.isExpression(e2) && e1.toJSON() === e2.toJSON(),
    replace : (toReplace,withThis) => {
        if ( withThis.parent !== null ) {
            const index = withThis.parent.children.indexOf( withThis )
            withThis.parent.children.splice( index, 1 )
            withThis.parent = null
        }
        if ( toReplace.parent === null ) return
        const index = toReplace.parent.children.indexOf( toReplace )
        toReplace.parent.children[index] = withThis
        withThis.parent = toReplace.parent
        toReplace.parent = null
    },
    isVariable : (e) => e.type() === 'v',
    getVariableName : (v) => v.text.substring(1),
    variable : (name) => BE( `_${name}` ),
    isApplication : (e) => e.type() === 'a',
    application : (es) => BE( [ 'a', ...es ] ),
    getChildren : (a) => a.children.slice( 1 ),
    isBinding : (e) => e.type() === 'b',
    binding : (head,vars,body) => BE( [ 'b', head, ...vars, body ] ),
    bindingHead : (b) => b.children[1],
    bindingVariables : (b) => b.children.slice( 2, b.children.length - 1 ),
    bindingBody : (b) => b.children[b.children.length - 1],
    variableIsFree : (v,e) => {
        for ( let walk = v ; walk ; walk = walk.parent ) {
            if ( API.isBinding( walk ) && API.bindingVariables( walk ).some(
                bound => API.equal( bound, v ) ) ) return false
            if ( walk === e ) break
        }
        return true
    },
    isMetavariable : (e) => API.isExpression(e) && e.text[0] === '*',
    setMetavariable : (e) => e.text = `*${e.text.substring(1)}`,
    clearMetavariable : (e) => e.text = `_${e.text.substring(1)}`,
    metaFlag : BE( 'metaFlag' )
};
const VAR = name => API.variable( name )
const MV = name => {
    const result = VAR( name )
    API.setMetavariable( result )
    return result
}
const APP = (...args) => API.application( args )
const BIN = (...args) => {
    const sym = args.shift()
    const body = args.pop()
    return API.binding( sym, args, body )
}
M.setAPI( API )
...
//         mc = new M.MatchingChallenge( [ MV('X'), VAR('a') ] );
//         expect(mc).toBeInstanceOf(M.MatchingChallenge);
...
//         constraints = [
//             [
//                 APP( VAR('and'), MV('P'), MV('Q') ),
//                 APP( VAR('and'), VAR('a'), APP( VAR('or'), VAR('b'), VAR('c') ) )
//             ]
//         ];
//         // console.log(constraints[0][0].toJSON());
//         // console.log(constraints[0][1].toJSON());
//         mc = new M.MatchingChallenge(...constraints);
//         console.log( 'START' )
            ...
            checkSolutions(
                ...
                [
                    new M.ConstraintList(
                        new M.Constraint( MV('P'), VAR('a') ),
                        new M.Constraint( MV('Q'), APP( VAR('or'), VAR('b'), VAR('c') ) )
                    )
                ]
                , true
            )
...
//         ).toBe(true);
        constraints = [
            [ BIN( VAR('forall'), MV('x'), EFA( MV('P'), MV('x') ) ),
              BIN( VAR('forall'), VAR('r'),
                   APP( VAR('>'), APP( VAR('+'), APP( VAR('sq'), VAR('r') ), VAR('1') ), VAR('0') ) ) ],
            [ EFA( MV('P'), MV('T') ),
              APP( VAR('>'), APP( VAR('+'), APP( VAR('sq'), VAR('-9') ), VAR('1') ), VAR('0') ) ]
        ];
        mc = new M.MatchingChallenge(...constraints);
        sols = mc.getSolutions();
        expect(
            checkSolutions(
                ...
                [
                    new M.ConstraintList(
                        new M.Constraint( MV('T'), VAR('-9') ),
                        new M.Constraint( MV('x'), VAR('r') ),
                        new M.Constraint(
                            MV('P'),
                            EF(
                                VAR('v'),
...
//     });
    ...
    test('should correctly solve challenges involving the equality elimination rule', () => {
        ...
        ////////// Test 11 //////////
        constraints = [
            [ APP( VAR('eq'), MV('a'), MV('b') ), APP( VAR('eq'), VAR('t'), VAR('1') ) ],
            [ EFA( MV('P'), MV('a') ), APP( VAR('gt'), VAR('t'), VAR('0') ) ],
            [ EFA( MV('P'), MV('b') ), APP( VAR('gt'), VAR('1'), VAR('0') ) ]
        ];
        mc = new M.MatchingChallenge(...constraints);
        sols = mc.getSolutions();
        expect(
            checkSolutions(
                ...
                [
                    new M.ConstraintList(
                        new M.Constraint( MV('a'), VAR('t') ),
                        new M.Constraint( MV('b'), VAR('1') ),
                        new M.Constraint( MV('P'), EF( VAR('v'), APP( VAR('gt'), VAR('v'), VAR('0') ) ) )
                    )
                ]
                , true
            )
...
//         constraints = newConstraints(
//             ['_P_of_0', 'gte(0,0)'],
//             ['for.all[_k,implies(_P_of__k,SecondOrderMatching.Meta(_P,plus(_k,1)))]',
